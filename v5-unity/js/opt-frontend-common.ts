...
export abstract class AbstractBaseFrontend {
  ...
  // Modern JavaScript/TypeScript focused configuration
  // Uses v4-cokapi backend for execution instead of local Python scripts
  langSettingToBackendScript = {
    js: "exec_js_jsonp",     // JavaScript execution via cokapi backend
    ts: "exec_ts_jsonp",     // TypeScript execution via cokapi backend
  };

  // these settings are all customized for my own server setup,
  // so you will need to customize for your server:
  serverRoot = "http://localhost:3000/"; // use local cokapi backend server

  // randomly pick one backup server to load balance:
  backupHttpServerRoot = "http://cokapi.com/"; // fallback to remote server
  ...
  abstract executeCode(
    forceStartingInstr?: number,
    forceRawInputLst?: string[]
  ): any;
  ...
  setFronendError(lines, ignoreLog = false) {
    $("#frontendErrorOutput").html(
      lines.map(htmlspecialchars).join("<br/>") +
        (ignoreLog
          ? ""
    ...
    if (!ignoreLog) {
      ...
      if (this.num414Tries > 0) {
        (myArgs as any).code =
          "#CodeTooLong: " + String((myArgs as any).code.length) + " bytes";
      }
  ...
  getBaseFrontendOptionsObj() {
    var ret = {
      ...
      disableHeapNesting:
        $("#heapPrimitivesSelector").val() == "true" ||
  ...
  executeCodeAndRunCallback(
    ...
    var deltaObjStringified =
      this.deltaObj && this.deltaObj.deltas.length > 0
      ...
      var retryOnBackupServer = () => {
        ...
        this.setFronendError([
          "Main server is busy or has errors; re-trying using backup server " +
            this.backupHttpServerRoot +
            " ... [#TryBackup]",
        ]);

        // now re-try the query using the backup server:
        var backup_jsonp_endpoint =
          this.langSettingToJsonpEndpointBackup[pyState];
