...
function traceExecution(code) {
  try {
    ...
    // Store reference to sandbox for variable tracking
    sandboxVars = sandbox;
    ...
    trace.push({
      ...
      func_name: "<module>",
      globals: captureVariables(sandboxVars),
      ordered_globals: Object.keys(captureVariables(sandboxVars)),
      stack_to_render: [],
      heap: {},
      stdout: output,
    });
...
function createEnhancedSandbox() {
  ...
  var sandbox = vm.createContext({
    ...
    __tracer: function (lineNumber) {
      ...
      // Capture current variables from the sandbox context
      var currentVars = captureVariables(this);
